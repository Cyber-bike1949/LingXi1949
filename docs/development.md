...
This is the single entry point for anyone setting up a local environment to work on LingXi: what the pieces are, how to build and test each of them, and the rules a contribution has to follow. (中文版见 [development_ZH.md](development_ZH.md)。)

## 1. Overview

LingXi is an Obsidian plugin that embeds a real terminal in a note-taking app, plus an optional remote-terminal feature: a Windows/desktop Obsidian control device drives a `termesh-agent` process on a target device (Windows or headless Ubuntu) over a direct `iroh` (QUIC) connection, no account or self-hosted server required.
...
- **No ads**, dynamic or static, anywhere outside LingXi's own UI surfaces (settings tab, its own modals/views), and only when genuinely related to LingXi itself.
