...
export function renderTermeshCodexSkill(): string {
  return [
    '---',
    `name: ${TERMY_CODEX_SKILL_NAME}`,
    'description: Use when a Codex session launched from the Magpie Bridge Obsidian plugin needs the current Obsidian note, selected text, active file, open files, vault root, workspace folders, or Magpie Bridge-provided Obsidian context. Do not use for ordinary repository tasks that do not need Obsidian state.',
    '---',
    '',
    '# Magpie Bridge Obsidian Context',
    '',
    TERMY_CODEX_SKILL_MANAGED_MARKER,
    '',
    'Use this skill to read the live Obsidian context snapshot exposed by Magpie Bridge.',
    '',
    `1. Read the JSON file path from \`${TERMY_CONTEXT_PATH_ENV}\`.`,
    `2. If \`${TERMY_CONTEXT_PATH_ENV}\` is missing or empty, state that Magpie Bridge context is unavailable and continue without guessing.`,
