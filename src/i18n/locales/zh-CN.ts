...
export const zhCN: TranslationKeys = {
  ...
  release21: {
    ...
    copyGroup: "复制为自定义命令组",
    copy: "副本",
    targetHint: "请确认目标操作系统和 Shell 后运行。WSL 请选择 Linux。已安装工具可能会升级。",
    targetPlatform: "目标操作系统",
    targetShell: "目标 Shell",
    unsupportedTarget: "请选择受支持的操作系统和 Shell。",
    feedback: "反馈与建议",
    feedbackHint: "报告问题或提出改进建议。",
    feedbackUrl: "请输入反馈表单的 HTTPS 地址",
    copyUrl: "打开失败，请复制选中的网址到浏览器。",
    deleteTitle: "删除此条目？",
    deleteHint: "删除文件夹会永久删除全部内容，无法撤销。",
    move: "移动",
    operationFailed: "文件操作失败：{{code}}",
    partialDelete: "部分内容已删除，请刷新检查剩余文件。",
    unknownResult: "执行结果未知，请刷新核对后再操作。",
    upgradeRequired: "此设备不支持当前位置的文件操作，请升级后端或检查允许根目录。",
    deleteType: "类型：{{type}}",
  },
  home: {
  ...
  remote: {
    ...
    transferCompleteAt: '已发送 {{count}} 个文件到 {{path}}',
    transferFailed: '笔记传输失败：{{message}}',
    sendToTerminalExecute: '发送到终端执行（含链接笔记）',
    sendToTerminalTooltip: '发送到终端（含链接笔记）',
    linkedNotesSkipped: '以下链接笔记未能收集，已跳过：{{details}}',
    transferQuotaExceededByBacklinks: '因包含反向链接笔记导致传输超出限额：{{message}}',
  },
  ...
  directoryTree: {
    ...
    close: '关闭目录树',
    empty: '空目录',
    pathInput: '当前路径（输入路径并按回车跳转）',
    copyToVault: '复制到 Vault',
    copyToVaultChooseFolderPlaceholder: '选择目标文件夹…',
    copyToVaultDone: '已复制到 Vault。',
    copyToVaultFailed: '复制到 Vault 失败：{{message}}',
    dropCopyDone: '已复制到 {{path}}。',
    dropCopyDoneWithSkips: '已复制到 {{path}}（另有 {{count}} 个因路径非法被跳过，详见日志）。',
    dropCopyFailed: '复制失败：{{message}}',
    remoteAgentTooOldForFolderTransfer: '该设备的 agent 版本过旧，不支持文件夹传输，请升级后重试。',
  ...
  notices: {
    ...
    downloadingRemoteRuntime: '正在下载远程终端运行库...',
    retryingRemoteRuntime: '正在通过 Obsidian 网络重试下载...',
    verifyingRemoteRuntime: '正在验证远程终端运行库...',
    remoteRuntimeReady: '远程终端运行库已就绪',
  ...
  settingsDetails: {
    terminal: {
      ...
      overwriteOnDuplicateFilename: '同名文件直接覆盖',
      overwriteOnDuplicateFilenameDesc: '将目录树上的文件拖拽到笔记上时，若目标位置已存在同名文件，直接覆盖而非追加序号。开启后同名文件将被直接覆盖，不可撤销；关闭则保持追加“(2)”等序号的原有行为。',
      sendBacklinkedNotes: '发送到终端时一并发送反向链接笔记',
      sendBacklinkedNotesDesc: '发送笔记到终端时，同时递归发送引用了这篇笔记的其它笔记，让 agent 拿到完整上下文。这会把当前笔记之外更多的笔记内容发送到远端设备；关闭后仅发送当前笔记及其正向链接到的内容。',
      transferConfirmThresholdFiles: '文件数超过此值时确认',
      transferConfirmThresholdFilesDesc: '发送到终端前，若收集到的文件数超过此值，会先弹窗确认。与下方的大小阈值任一超出都会触发确认。',
      transferConfirmThresholdSize: '总大小超过此值（MB）时确认',
      transferConfirmThresholdSizeDesc: '发送到终端前，若收集到的文件总大小（MB）超过此值，会先弹窗确认。',
      ...
      hideUnavailableAiLaunchersDesc: '隐藏 CLI 未安装在本机上的内置 AI 启动器（Claude Code、Codex、OpenCode）。仅影响状态栏菜单。',
      checkAiLauncherUpdates: '检查 AI 启动器更新',
      checkAiLauncherUpdatesDesc: '将本地安装的 Claude Code、Codex 与 OpenCode 版本与上游注册表（npm 与 GitHub 发布版本）进行对比。默认关闭；离线模式下也会忽略此开关。',
  ...
  modals: {
    ...
    transferConfirm: {
      title: '确认发送？',
      body: '即将发送 {{count}} 个文件，共 {{size}}，发送到终端。是否继续？',
      sendAnyway: '仍然发送',
    },
  },
